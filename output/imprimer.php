<?php

/*
 * Depuis ecritureBD.php   "n" est mis dans une variable session avant d'être transmis(via l'url) ici
 *
 *  $id_document = $_SESSION['id_document'] ?? "";
 *
 * <a  href="imprimer.php?n=<?php echo $id_document; ?> "  >
 *      <input type="button"  value="Imprimer l'acte" />
 * </a>
 *
 * D'autres pages ont transmis un identifiant ici ⚠️
 *
 */

if(!isset($_GET['n'])) $_GET['n']=""; // Transmis par ecritureBD.php ou 'backend/colonne-afficher-naissance.php' , ...etc
$id=$_GET['n'];
$id=ltrim($id);
// try{$conn = new PDO('mysql:host=localhost;dbname=etatcivil;charset=utf8', 'root', '',array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));} //// Le array active les exception PDO: pour obtenir + de détail sur d'eventuels erreurs
// catch(Exception $e){die('Erreur de connexion à la base de données: '.$e->getMessage());}
require_once '../backend/connection_PDO.php';

if($id <= 0) {
    die("ID invalide : \n <h4>Veuillez saisir le document avant de l'imprimer ⚠️</h4>");
}


$reponse = $conn->query('SELECT * FROM liste WHERE ID='.$id );
$donnees = $reponse->fetch();
?>

<!DOCTYPE html>
<html>
<head>
     <meta charset="utf-8"> <!-- sinon tu peux pas écrire N° ni les accent-->
	 <!-- <meta name="viewport" content="width=device-width, initial-scale=1.0, roles-scalable=yes"> -->
	 <title>Recupere l'identifiant transmis et l'afffiche dans un pop </title>
	 <!-- Attention! afficher.css je le trouve pas ⚠️⚠️⚠️  -->
	 <link href="../css/afficher.css" rel="stylesheet" title="Style" /> 
     <style>
	    input { border:none; }	
		
		.titregauche,
	    .titredroite{
		   text-align:center;
	    }
	    .titregauche{
		   padding-inline:.3em;
	    }
		
		.nomPrefecture{
			margin-left:-95px;
			margin-bottom:0;
			padding-bottom:0;
		}
		
		.registre{
			margin-left:65px;
		}
		.naissanceDe{
			margin-left:85px;
		}
		
		#minuite{
			margin-left:300px;
			width:20px;
			position:absolute;
		}
		.naissance_x{
			margin-left:120px;
		}
		 
		 
		/* ##################### 1er réajustement de la sortie  ##############################  */ 
         
         /* Supprimer les marge */
		 
	     @page { margin: 0; }
         body { margin: .5em 0 0 0 !important; }
		 
		 /* Remplire la page */
		 
         @media (max-width: 768px) {
			 body{
				 min-width:98vw !important;
				 min-height:98vh !important;
			 }
		 }

		 
         /* ##################### 2em réajustement de la sortie  ##############################  */
		 
         /* 🟩 Ajustement verticale */
           #wrap{
			   line-height:1.92;
		   }
		   
        /* 🟩 Ajustement Horizontale */
		   #wrap{
			   padding:0 !important;
		   }
		   table{
			   width:97% !important;
		   }
		   .tdUn{
			   width:40% !important;
		   }



	  
     </style>	 
</head>
<!-- Tasks: tests et le reajustement CSS3 selon le resultat des  tests -->
<body onLoad="window.print()">
<div id="wrap">

<table border="1" align="center"  style="border-collapse:collapse" bordercolor="#111111" width="80%" >
  
   <tr>
      <td align="center" VALIGN="top" class="tdUn">
	       <h2 class="titregauche">UNION DES COMORES</h2> 
		   <h6>Unit&eacute;-Solidarit&eacute;-D&eacute;veloppement<h6> 
		   <h3>MINISTÈRE DE L&apos;INTÉRIEUR</h3> 
		   <img src="../img/armoirie.png"  /> 
	  
           <h4> Pr&eacute;fecture de: </h4> 		   
		   <input type="text"  class="nomPrefecture"   value="<?php echo $donnees["prefecture"];?>"  >		   
		   <hr style="margin-top:0;" />
		   
		   <h4>Centre d'État Civil de: </h4>  
		   <input type="text" class="nomPrefecture"  value="<?php echo $donnees["centretatcivil"];?>" > 
		   <hr style="margin-top:0;" /> 
		   <div style="margin-left:10px;">
		        <h6 align="left" style="line-height:0;">			    
				      <input type="text"   class="registre"  value="<?php echo $donnees["registre"];?>" ><br>
				      Registre N° -----------------------------------------------------------------				 
				</h6>
				<h6 align="left">
				    <p style="line-height:0; ">
				      <input type="text"   class="registre"  value="<?php echo $donnees["acte"];?>" ><br>
				      Acte N° ----------------------------------------------------------------------</h6><h6 align="left" >
				    </p>
				    <p style="line-height:0; ">
				      <input type="text"  class="registre"  value="<?php echo $donnees["date_acte"];?>" ><br>
				      Du ----------------------------------------------------------------------------
					</p>
				</h6>
			</div>
	  </td>
      
	  <td  align="left"  rowspan="2" >
	     <h1 align="center"  class="titredroite" style="color: white; text-shadow: 2px 2px 4px #1D702D;">ACTE DE NAISSANCE</h1>
		 <div style="margin-left:10px;">
		 <p style="line-height:0; ">
	        <input type="text"  class="naissanceDe"  value="<?php echo $donnees["naissance_jour_moi"];?>"  ><br><b>
		    <span>Le</span><span> ------------------------------------------------------------------------------</span><br>
		 </p>
		 <p style="line-height:0;">		 
		    <input type="text"  class="naissanceDe"  value="<?php echo $donnees["naissance_an"];?>" ><br>
		    et l'an -------------------------------------------------------------------------<br>
		 </p>
		 <p style="line-height:0; ">
		    <input type="text" id="minuite"  value="<?php echo $donnees["naissance_minuite"];?>">
		    <input type="text"  style="margin-left:90px;" value="<?php echo $donnees["naissance_heure"];?>"><br>
			&agrave; --------------------------- heure(s) --------------------------- minute(s)<br><br>
		 </p>
		 <p style="line-height:0; margin-top:3em; ">
		    <input type="text"  class="naissance_x" value="<?php echo $donnees["naissance_nom_prenom"];?>"><br>
		    est n&eacute;(e) -----------------------------------------------------------------------<br>
		 </p>
		 <p style="line-height:0; ">
		    <input type="text"  class="naissance_x" value="<?php echo $donnees["naissance_lieu"];?>"><br>
		    &agrave; --------------------------------------------------------------------------------<br>
		 </p>
		 <p style="line-height:0; ">
		    <input type="text" class="naissance_x" value="<?php echo $donnees["naissance_sexe"];?>"><br>
		    du sexe ------------------------------------------------------------------------<br><br><br>
		 </p>
         <p style="line-height:0; margin-top:3em;  ">		
		    <input type="text"  class="naissance_x"  value="<?php echo $donnees["pere_nom_prenom"];?>"><br>
		    fils(fille de) -------------------------------------------------------------------<br>
		 </p> 
		 <p style="line-height:0; ">
		    <input type="text"  class="naissance_x" value="<?php echo $donnees["pere_datenaisance"];?>"><br>
		    n&eacute(e) le ------------------------------------------------------------------------<br>
		 </p>
		 <p style="line-height:0; ">
		    <input type="text"  class="naissance_x" value="<?php echo $donnees["pere_lieunaissance"];?>" ><br>
		    &agrave; --------------------------------------------------------------------------------<br>
		 </p>
		 <p style="line-height:0; ">
		    <input type="text"  class="naissance_x" value="<?php echo $donnees["pere_profession"] ;?>" ><br>
		    Profession --------------------------------------------------------------------<br>
		 </p>
		 <p style="line-height:0; ">
		    <input type="text"  class="naissance_x" value="<?php echo $donnees["pere_villederesidence"] ;?>" ><br>
		    Démeurant &agrave; -----------------------------------------------------------------<br>
		 </p>
		 <p align="left" style="margin-top:3em;" >
		    <p style="line-height:0; ">
			    <input type="text"  class="naissance_x" value="<?php echo $donnees["mere_nom_prenom"];?>"><br>
			    et de ---------------------------------------------------------------------------<br>
			</p> 
			<p style="line-height:0; ">
			   <input type="text"  class="naissance_x" value="<?php echo $donnees["mere_datenaisance"];?>" ><br>
			   n&eacute;(e) le ------------------------------------------------------------------------<br>
			</p>
			   <p style="line-height:0; ">
			   <input type="text"  class="naissance_x" value="<?php echo $donnees["mere_lieunaissance"];?>" ><br>
			   &agrave; -------------------------------------------------------------------------------<br>
			</p>
			<p style="line-height:0; ">
			  <input type="text"  class="naissance_x" value="<?php echo $donnees["mere_profession"] ;?>" ><br>
			  Profession --------------------------------------------------------------------<br>
			</p>
			<p style="line-height:0; padding-bottom:1em;">
			  <input type="text"  class="naissance_x" value="<?php echo $donnees["mere_villederesidenc"] ;?>" ><br>
			  Démeurant &agrave; ----------------------------------------------------------------
	        </p>
		 </p>
		 
		 </div>
	  </td>
   </tr>
   
   <tr>
      <td align="center" rowspan="2"  >
	        <h4> Naissance de:</h4> <br>
			
			   <input type="text"  style="margin-left:-95px; margin-bottom:0;"  value="<?php echo $donnees["nom"];?>">
			   <hr style="margin-top:0;" />
			
			
			   <input type="text"  style="margin-left:-95px; margin-bottom:0;"  value="<?php echo $donnees["prenom"];?>">
			   <hr style="margin-top:0;" /><br>
	  </td>
   </tr>
   
   <tr height="20px"><td  rowspan="2">
         <p align="left">
	    <div style="margin-left:10px; line-height:0;">
		  
	      <h4 align="center"> D&eacute;claration faite par:</h4> 
		 
		  <input type="text"  style="margin-left:85px; " value="<?php echo $donnees["declaration_faite_par"];?>" >		  
		  <br>----------------------------------------------------------------------------------------------------------------------<br> 
		 <p>
		  <input type="text"  style="margin-left:85px;"  value="<?php echo $donnees["datejugement"];?>"  >
		  <br>----------------------------------------------------------------------------------------------------------------------<br>
         </p> 
		  
		  <h4 align="center"> D&eacute;claration re&ccedil;ue le:</h4> 
		  <input type="text"  style="margin-left:85px;" value="<?php echo $donnees["declaration_recue_pa"];?>" >
		  <br>----------------------------------------------------------------------------------------------------------------------<br>

		  <br>----------------------------------------------------------------------------------------------------------------------<br><br>
	    </div>
	  </p>
   </td></tr>
   
   <tr>
      <td align="left" >
	     <h4 align="center"> Pour acte certifi&eacute; conforme:</h4> 
		 <div style="margin-left:10px;">
		 <p style="line-height:0; ">
		    <input type="text"  style="margin-left:95px;"  value="<?php echo $donnees["delivre_a"];?>" ><br><b>
		    D&eacute;livr&eacute; &agrave; -----------------------------------------<br>
		</p>
		<p style="line-height:0; ">
		    <input type="text"  style="margin-left:95px;"  value="<?php echo $donnees["delivre_le"];?>" ><br>
		    Le -------------------------------------------------<br>
		</p>
		<p style="line-height:0; ">
		   <input type="text"  style="margin-left:95px;"   value="<?php echo $donnees["delivre_an"];?>"><br>
		   L'an -----------------------------------------------<br>
		 </p>
		 </div>
		 <p > <h4  align="center"> L&apos;officier de l'État Civil:</h4></p>
	  </td>
   </tr>
</table>
</div>
</body>
</html>
