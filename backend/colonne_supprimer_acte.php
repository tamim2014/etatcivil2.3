...
while($ligne2=mysqli_fetch_array($R)){// en utlisant FOREACH ça marche pas .j'sais pas pourquoi
	 $table.='<tr><td>'.$ligne2["nom"].'</td><td>'.$ligne2["prenom"].'</td><td>'.$ligne2["acte"].'</td><td>'.$ligne2["prefecture"].'</td> <td> <a id="s1" href="backend/supprimer.php?n='.$ligne2["ID"].'">Supprimer</a> </td></tr>';
	 }
$table.='</table>';
echo $table;
mysqli_close($conn);
